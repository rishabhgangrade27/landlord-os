...
export default async function MaintenanceDetailPage({
  ...
  const { data: ticket } = await supabase
    ...
    .select(`
      *,
      ...
      contractors:assigned_contractor_id(id, name, phone, email, trade)
    `)
    .eq('id', id)
    .single()
  ...
  const unit = (ticket as any).units
  const contractor = (ticket as any).contractors
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {[
                ...
                { label: 'Unit', value: unit ? `${unit.properties?.name ?? unit.properties?.address} / ${unit.unit_number}` : null },
