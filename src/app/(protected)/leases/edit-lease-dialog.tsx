...
import { getPropertyOptions, updateLease } from './actions'
...
export function EditLeaseDialog({
  ...
  const [properties, setProperties] = useState<{id: string, name: string | null, nickname: string | null}[]>([])
  ...
  if (open && properties.length === 0 && !loading) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { success, error } = await updateLease(lease.id, {
                ...
                <SelectContent>
                  ...
                  {properties.map(p => (
                    <SelectItem key={p.id} value={p.id}>
                      {p.nickname ?? p.name ?? 'Unnamed Property'}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
