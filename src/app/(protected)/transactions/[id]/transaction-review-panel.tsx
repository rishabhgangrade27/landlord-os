...
import { updateTransaction, deleteTransaction, createTenant } from './actions'
...
export function TransactionReviewPanel({
  ...
  async function handleDelete() {
    ...
    setRejecting(true) // Reuse state for loading
  ...
  async function handleAddTenant() {
    ...
    const { data, error } = await createTenant({ name: addTenantForm.name.trim(), case_number: addTenantForm.case_number.trim() })
    if (error || !data) {
  ...
  async function handleReset() {
    const { error } = await updateTransaction(transaction.id, { status: 'needs_review' })
    if (error) toast.error(error.message)
    else {
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="pt-2">
            <Button
              ...
              className="text-destructive h-auto p-0 hover:text-destructive"
            >
