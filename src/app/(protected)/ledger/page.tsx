...
async function TenantLedger({ tenantId }: { tenantId: string }) {
  const [
    ...
    prisma.$queryRaw<any[]>`SELECT * FROM view_court_ledger WHERE tenant_id = ${tenantId} ORDER BY ledger_month, check_date`,
    prisma.$queryRaw<any[]>`SELECT * FROM view_rent_ledger WHERE tenant_id = ${tenantId} ORDER BY month`
  ])

  const latestCourtPdfJobs = await prisma.$queryRaw<any[]>`
    SELECT pdf_url FROM pdf_jobs 
    WHERE job_type = 'court_ledger' 
      AND reference_id = ${tenantId} 
      AND status = 'done' 
    ORDER BY completed_at DESC 
    LIMIT 1
  `
  const latestCourtPdfJob = latestCourtPdfJobs?.[0]
  ...
  for (let i = 0; i < sortedLeases.length - 1; i++) {
    const gapStart = new Date(sortedLeases[i].end_date!.toISOString().slice(0, 10) + 'T00:00:00')
    const gapEnd   = new Date(sortedLeases[i + 1].start_date.toISOString().slice(0, 10) + 'T00:00:00')
    gapStart.setDate(1)
                    ...
                    <div className="flex flex-wrap gap-2">
                      {allLeases.map((l: any, i: number) => (
                        <span key={i} className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border ${l.status === 'active' ? 'bg-green-50 border-green-300 text-green-800' : 'bg-muted border-muted-foreground/20 text-muted-foreground'}`}>
                          ...
                          <span>·</span>
                          <span>{toDateKey(l.start_date)} – {l.end_date ? toDateKey(l.end_date) : 'ongoing'}</span>
          ...
          <tbody>
            ...
            {allLeases.length > 0 && (
              <tr>
                ...
                <td colSpan={3}>
                  {allLeases.map((l: any, i: number) =>
                    `${toDateKey(l.start_date)} – ${l.end_date ? toDateKey(l.end_date) : 'Ongoing'} ($${Number(l.rent_amount).toLocaleString()}/mo)${i < allLeases.length - 1 ? ' | ' : ''}`
                  ).join('')}
                </td>
              </tr>
            )}
...
export default async function LedgerPage({
  ...
  const properties = await prisma.property.findMany({
    select: {
      id: true, name: true, nickname: true, address: true, city: true, state: true, status: true,
      leases: {
        select: {
          id: true, rent_amount: true, status: true,
          tenant: { select: { id: true, name: true, case_number: true, status: true } }
        }
      }
    },
  ...
  const allLedger = await prisma.$queryRaw<any[]>`SELECT tenant_id, pending_balance, month FROM view_rent_ledger ORDER BY month DESC`
