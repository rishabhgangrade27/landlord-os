...
export default async function PropertyDetailPage({
  ...
  // Retrieve ledger balance using raw query if available, otherwise fallback to empty
  let ledgerRows: any[] = []
  try {
    ledgerRows = await prisma.$queryRaw`SELECT tenant_id, pending_balance FROM view_rent_ledger WHERE property_id = ${id} ORDER BY month DESC`
  } catch (e) {}
            ...
            {activeTenant ? (
                ...
                <div className="flex-1">
                  ...
                  {activeLease && (
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Lease: {activeLease.start_date.toISOString().split('T')[0]} → {activeLease.end_date ? activeLease.end_date.toISOString().split('T')[0] : 'Ongoing'}
                    </p>
                  )}
                  {(() => {
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Last lease ended: <strong>{mostRecentLease!.end_date ? mostRecentLease!.end_date.toISOString().split('T')[0] : 'unknown'}</strong>
                  </p>
                      ...
                      {leases.map((lease) => {
                        ...
                        return (
                          <tr key={lease.id} className="border-b last:border-0 hover:bg-muted/10">
                            ...
                            <td className="px-4 py-2.5 text-muted-foreground hidden sm:table-cell">
                              {lease.end_date ? lease.end_date.toISOString().split('T')[0] : 'Ongoing'}
                            </td>
