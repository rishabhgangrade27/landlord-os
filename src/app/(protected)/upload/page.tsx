...
import { uploadReceiptFile } from './actions'
...
export default function UploadPage() {
  ...
  async function uploadAll() {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const res = await uploadReceiptFile(formData)

      if (res.error) {
        setFiles((prev) =>
          prev.map((f, idx) =>
            idx === i ? { ...f, status: 'error', progress: 0, error: res.error } : f
          )
        )
        toast.error(`Failed: ${file.name} — ${res.error}`)
      } else {
        setFiles((prev) =>
          prev.map((f, idx) =>
            idx === i ? { ...f, status: 'done', progress: 100, path: res.path } : f
          )
        )
      }
    }
