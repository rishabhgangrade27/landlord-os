...
import { getTenantOptions, getPropertyOptions, checkActiveLeaseForProperty, createLease } from '../actions'
...
type PropertyOption = { id: string; nickname: string | null; address: string | null; status: string | null }
type ActiveLeaseWarning = { tenant_name: string; start_date: Date; rent_amount: number }
...
function NewLeasePageContent() {
  ...
  useEffect(() => {
    getTenantOptions().then(setTenants)
    getPropertyOptions().then(setProperties)
  }, [])

  async function checkForActiveLease(propertyId: string) {
    if (!propertyId) { setActiveLeaseWarning(null); return }
    const warning = await checkActiveLeaseForProperty(propertyId)
    setActiveLeaseWarning(warning)
  }
                ...
                {activeLeaseWarning && (
                  <div className="flex items-start gap-2 rounded-md border border-yellow-300 bg-yellow-50 dark:border-yellow-700 dark:bg-yellow-950 px-3 py-2 text-sm text-yellow-800 dark:text-yellow-200">
                    ...
                    <span>
                      ...
                      {new Date(activeLeaseWarning.start_date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}.
